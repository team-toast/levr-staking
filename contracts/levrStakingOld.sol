...
contract StakingRewardsOld is /* IStakingRewards, */ RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    mapping(address => uint256) public depositDate; // Modified E
        ...
        updateReward(receiver) 
    {
        ...
        // reward = reward.add(reward.mul(5).div(1000).mul(block.timestamp - depositDate[receiver]));  // Modified E
        if (reward > 0) {
            rewards[receiver] = 0;
            if(overGracePeriod) {
